...
namespace yolo
{
    ...
    {
    public:
        YOLO(const util::InitParameter& param);
        ~YOLO();
    ...
    public:
        std::vector<std::vector<util::Box>> getObjectss() const;
    ...
    protected:
        util::InitParameter m_param;
        nvinfer1::Dims m_output_dims;   
        int m_output_area;
        int m_total_objects;
        std::vector<std::vector<util::Box>> m_objectss;
        util::AffineMat m_dst2src;     
