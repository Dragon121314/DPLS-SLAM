...
{
public:
	YOLOv8Seg(const util::InitParameter& param);
	~YOLOv8Seg();
...
public:
	void showAndSave(const std::vector<std::string>& classNames, const int& cvDelayTime, std::vector<cv::Mat>& imgsBatch);
	void getInstanceMask(cv::Mat &instance, std::vector<util::Box> &segbox);
