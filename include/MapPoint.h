...
public:
    ...
    // Variables used by the tracking
    float mTrackProjX,mTrackProjY;//该地图点投影在当前左图像的像素横纵坐标
    float mTrackDepth,mTrackDepthR;//该地图点在当前相机坐标系下的深度，是在Frame.cc的isInFrustum函数中计算出来的
    float mTrackProjXR,mTrackProjYR;//该地图点投影在当前右图像的像素横纵坐标
    bool mbTrackInView, mbTrackInViewR;//外点为fales,表示不显示，决定一个地图点是否进行重投影的标志
    int mnTrackScaleLevel, mnTrackScaleLevelR;//在图像金字塔中的哪一层
    float mTrackViewCos, mTrackViewCosR;//当前视角和法线(平均观测方向)夹角的余弦值
    long unsigned int mnTrackReferenceForFrame;//表示它已经是当前帧的局部地图点了，可以防止重复添加局部地图点
    long unsigned int mnLastFrameSeen;//地图点上一次被观察到的帧ID，之后由于变成外点不显示
