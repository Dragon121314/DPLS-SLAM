...
//============================================
    ...
    vector<MapLine*> GetReferenceMapLines()
    {
        unique_lock<mutex> lock(mMutexMap);
        return mvpReferenceMapLines;
    }
    void SetReferenceMapLines(const vector<MapLine *> &vpMLs)
    {
        unique_lock<mutex> lock(mMutexMap);
...
protected:
    ...
    long unsigned int mnInitKFid;//地图的初始关键帧ID
