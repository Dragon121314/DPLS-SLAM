...
{
public:
    Point(const float &acc_x, const float &acc_y, const float &acc_z,const float &ang_vel_x, const float &ang_vel_y, const float &ang_vel_z,
...
public:
    ...
    Eigen::Matrix3f deltaR; //由角速度计算得到的旋转矩阵
...
public:
    float dT;//IMU前后两图像帧的时间间隔
    Eigen::Matrix<float,15,15> C;
    Eigen::Matrix<float,15,15> Info;
    Eigen::DiagonalMatrix<float,6> Nga, NgaWalk;//加速度计和陀螺仪的噪声和随机游走

    // Values for the original bias (when integration was computed)
    Bias b;//偏置，包括陀螺仪和加速度计
    Eigen::Matrix3f dR;//旋转矩阵增量
    Eigen::Vector3f dV, dP;//速度、位置增量
    Eigen::Matrix3f JRg, JVg, JVa, JPg, JPa;//状态变量对偏置的雅可比矩阵
    Eigen::Vector3f avgA, avgW;//两图像帧之间的平均加速度和平均角速度
...
private:
    ...
    std::vector<integrable> mvMeasurements;//imu数据
