...
/** 
 * @brief 单目初始化重要的环节，先获得R，t再通过三角化恢复3d点坐标
 ...
 */
bool TwoViewReconstruction::Reconstruct(const std::vector<cv::KeyPoint> &vKeys1, const std::vector<cv::KeyPoint> &vKeys2, const vector<int> &vMatches12,Sophus::SE3f &T21, vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
{
    mvKeys1.clear();// 1. 准备工作，提取匹配关系及准备RANSAC
    mvKeys2.clear();
    mvKeys1 = vKeys1;
    mvKeys2 = vKeys2;
    // Reference Frame: 1, Current Frame: 2。填写mvMatches12，里面存放的是vKeys1，vKeys2匹配点的索引
    mvMatches12.clear();
    ...
    vector<size_t> vAllIndices;// 使用vAllIndices为了保证8个点选不到同一个点
    vAllIndices.reserve(N);
    vector<size_t> vAvailableIndices;
    for (int i = 0; i < N; i++) vAllIndices.push_back(i);
    // Generate sets of 8 points for each RANSAC iteration
    mvSets = vector<vector<size_t>>(mMaxIterations, vector<size_t>(8, 0));// 默认200次
    DUtils::Random::SeedRandOnce(0);
    for (int it = 0; it < mMaxIterations; it++)// 2. 先遍历把200次先取好
    {
        vAvailableIndices = vAllIndices;
        for (size_t j = 0; j < 8; j++)//选择一个八点法的最小集合
        {
            int randi = DUtils::Random::RandomInt(0, vAvailableIndices.size() - 1);
            int idx = vAvailableIndices[randi];  // 这句不多余，防止重复选择
            mvSets[it][j] = idx;
            vAvailableIndices[randi] = vAvailableIndices.back(); // 保证选不到同一个点，这么做的话可以删去vAvailableIndices已选点的索引
            vAvailableIndices.pop_back();
        }
    }
    vector<bool> vbMatchesInliersH, vbMatchesInliersF;
    float SH, SF;
    Eigen::Matrix3f H, F;//启动两个线程并行计算基础矩阵和单应性矩阵
    ...
    threadH.join();// Wait until both threads have finished
    threadF.join();
    if (SH + SF == 0.f) return false;// Compute ratio of scores
    float RH = SH / (SH + SF);
    float minParallax = 1.0;
    // 4. 看哪个分高用哪个，分别是通过H重建与通过F重建
    if (RH > 0.50) return ReconstructH(vbMatchesInliersH, H, mK, T21, vP3D, vbTriangulated, minParallax, 50);
    else return ReconstructF(vbMatchesInliersF, F, mK, T21, vP3D, vbTriangulated, minParallax, 50);
 ...
 */
bool TwoViewReconstruction::ReconstructF(vector<bool> &vbMatchesInliers, Eigen::Matrix3f &F21, Eigen::Matrix3f &K,Sophus::SE3f &T21, vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated, float minParallax, int minTriangulated)
{
 ...
 * @param minTriangulated 50
 */
bool TwoViewReconstruction::ReconstructH(vector<bool> &vbMatchesInliers, Eigen::Matrix3f &H21, Eigen::Matrix3f &K,Sophus::SE3f &T21, vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated, float minParallax, int minTriangulated)
{
    // 统计了合法的匹配，后面用于对比重建出的点数
    int N = 0;
    for (size_t i = 0, iend = vbMatchesInliers.size(); i < iend; i++)
