...
void MapPoint::AddObservation(KeyFrame* pKF, int idx)
{
    ...
    if(mObservations.count(pKF)) indexes = mObservations[pKF];
    else indexes = tuple<int,int>(-1,-1);

    if(pKF -> NLeft != -1 && idx >= pKF -> NLeft) get<1>(indexes) = idx;
    else get<0>(indexes) = idx;
    // 如果没有添加过观测，记录下能观测到该MapPoint的KF和该MapPoint在KF中的索引
    mObservations[pKF]=indexes;
    if(!pKF->mpCamera2 && pKF->mvuRight[idx]>=0) nObs+=2;
    else nObs++;
}
...
void MapPoint::ComputeDistinctiveDescriptors()
{
    ...
    for(size_t i=0;i<N;i++)
    {
        vector<int> vDists(Distances[i],Distances[i]+N); // 第i个描述子到其它所有所有描述子之间的距离
        sort(vDists.begin(),vDists.end());
        int median = vDists[0.5*(N-1)];// 获得中值
        if(median<BestMedian)// 寻找最小的中值
        {
            BestMedian = median;
            BestIdx = i;
        }
...
{
    map<KeyFrame*,tuple<int,int>> observations;// 获得观测到该地图点的所有关键帧
    KeyFrame* pRefKF;// 观测到该点的参考关键帧（第一次创建时的关键帧）
    Eigen::Vector3f Pos; // 地图点在世界坐标系中的位置
    {
        unique_lock<mutex> lock1(mMutexFeatures);
        unique_lock<mutex> lock2(mMutexPos);
        if(mbBad) return;
    ...
    int leftIndex = get<0>(indexes), rightIndex = get<1>(indexes);
    int level;//地图点在图像金字塔中的层数
    if(pRefKF -> NLeft == -1) level = pRefKF->mvKeysUn[leftIndex].octave;
    else if(leftIndex != -1) level = pRefKF -> mvKeys[leftIndex].octave;
    else level = pRefKF -> mvKeysRight[rightIndex - pRefKF -> NLeft].octave;
    ...
    {
        unique_lock<mutex> lock3(mMutexPos); // 使用方法见PredictScale函数前的注释
