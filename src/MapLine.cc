    ...
    mbBad(false), mpReplaced(static_cast<MapLine*>(NULL)), mpMap(pMap)
{
    ...
    mfMaxDistance = dist*1.2;
    mfMinDistance = mfMaxDistance/1.2; 
...
void MapLine::ComputeDistinctiveDescriptors()
{
    ...
    for(map<KeyFrame*,size_t>::iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
    {
        ...
        if(!pKF->isBad()) vDescriptors.push_back(pKF->mDescriptors_l.row(mit->second));
    }
...
// cout<<"0"<<endl;
    ...
    cv::Mat Pos;
    {
        ...
        observations=mObservations;
        pRefKF=mpRefKF;
        Eigen::Vector3f MidPoint = (mWorldPos_sP+mWorldPos_eP)/2;
