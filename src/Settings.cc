...
Settings::Settings(const std::string &configFile, const int &sensor) : bNeedToUndistort_(false), bNeedToRectify_(false), bNeedToResize1_(false), bNeedToResize2_(false)
{
    ...
    else cout << "Loading settings from " << configFile << endl;
    readCamera1(fSettings);// Read first camera
...
void Settings::readCamera1(cv::FileStorage &fSettings)
{
    ...
    if (cameraModel == "PinHole")
    {
        ...
        if (found) // Check if it is a distorted PinHole。加载相机畸变参数
        {
        ...
        if((sensor_==System::MONOCULAR||sensor_==System::IMU_MONOCULAR||sensor_==System::RGBD||sensor_ == System::IMU_RGBD) 
                && vPinHoleDistorsion1_.size()!= 0) bNeedToUndistort_ = true;
    }
...
void Settings::readCamera2(cv::FileStorage &fSettings)
{
    ...
    if (cameraType_ == PinHole)
    {
        ...
        if (found)
        {
            ...
            else {vPinHoleDistorsion2_.resize(4);}
...
void Settings::precomputeRectificationMaps()
{
    ...
    cv::Mat R_r1_u1, R_r2_u2;//分别是相机1和相机2的旋转矩阵，用于将相机坐标系转换到新的校正坐标系。
    cv::Mat P1, P2, Q;//分别是相机1和相机2的投影(内参)矩阵。Q用于计算视差图的矩阵。

    cv::stereoRectify(K1, camera1DistortionCoef(), K2, camera2DistortionCoef(), newImSize_,
                        R12, t12,
                        R_r1_u1, R_r2_u2, P1, P2, Q,
                        cv::CALIB_ZERO_DISPARITY, -1, newImSize_);//cv::CALIB_ZERO_DISPARITY：表示视差图中的视差为零时，对应的是图像中心。

    //使用OpenCV的函数来计算立体相机的校正参数，然后生成映射矩阵，以便对两个相机的图像进行畸变校正和重映射，使得它们可以在水平方向上对齐，便于后续的立体视觉处理。                   
    cv::initUndistortRectifyMap(K1, camera1DistortionCoef(), R_r1_u1, P1.rowRange(0, 3).colRange(0, 3),
                                newImSize_, CV_32F, M1l_, M2l_);// M1l_, M2l_输出的两个映射矩阵，将用于重映射相机1的图像。
