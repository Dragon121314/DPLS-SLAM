...
{
    list<KeyFrame *> lKFsSharingWords;//和当前关键帧拥有共同单词的所有关键帧列表
    {
        unique_lock<mutex> lock(mMutex);
        for (DBoW2::BowVector::const_iterator vit = F->mBowVec.begin(), vend = F->mBowVec.end(); vit != vend; vit++)
        {
            list<KeyFrame *> &lKFs = mvInvertedFile[vit->first];//包含当前帧某一个单词的所有关键帧列表
            for (list<KeyFrame *>::iterator lit = lKFs.begin(), lend = lKFs.end(); lit != lend; lit++)
            {
                KeyFrame *pKFi = *lit;
    ...
    int maxCommonWords = 0;//和当前关键帧拥有最多的共同单词数
    for (list<KeyFrame *>::iterator lit = lKFsSharingWords.begin(), lend = lKFsSharingWords.end(); lit != lend; lit++)
        if ((*lit)->mnRelocWords > maxCommonWords) maxCommonWords = (*lit)->mnRelocWords;
    int minCommonWords = maxCommonWords * 0.8f;
    list<pair<float, KeyFrame *>> lScoreAndMatch;//与当前关键帧拥有足够多共同单词数的关键帧及其相似度评分
    int nscores = 0;
    for (list<KeyFrame *>::iterator lit = lKFsSharingWords.begin(), lend = lKFsSharingWords.end(); lit != lend; lit++)
    {
        KeyFrame *pKFi = *lit;
    ...
    list<pair<float, KeyFrame *>> lAccScoreAndMatch;//记录拥有足够多共同单词的关键帧的共视关键帧里面的累积相似度评分及组里面相似度评分最高的那个共视关键帧
    float bestAccScore = 0;//记录拥有足够多共同单词的关键帧的10个共视关键帧作为一组里面累积相似度分数最高的
    // Lets now accumulate score by covisibility
    for (list<pair<float, KeyFrame *>>::iterator it = lScoreAndMatch.begin(), itend = lScoreAndMatch.end(); it != itend; it++)
    {
        KeyFrame *pKFi = it->second;
        vector<KeyFrame *> vpNeighs = pKFi->GetBestCovisibilityKeyFrames(10);
        float bestScore = it->first;//共视关键帧组里面最高的相似度评分
        float accScore = bestScore;
        KeyFrame *pBestKF = pKFi;//相似度评分最高的那个共视关键帧
        for (vector<KeyFrame *>::iterator vit = vpNeighs.begin(), vend = vpNeighs.end(); vit != vend; vit++)
        {
            KeyFrame *pKF2 = *vit;
    ...
    float minScoreToRetain = 0.75f * bestAccScore;//共视关键帧组最小累积相似度评分
    set<KeyFrame *> spAlreadyAddedKF;
    vector<KeyFrame *> vpRelocCandidates;//重定位的候选帧
