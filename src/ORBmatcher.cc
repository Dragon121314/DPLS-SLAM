 ...
 * @param F 当前帧
 * @param vpMapPoints 局部地图点集合
 * @param th  投影匹配搜索的阈值范围
 * @param bFarPoints  是否考虑远点
 * @param thFarPoints 远点的阈值范围
*/
int ORBmatcher::SearchByProjection(Frame &F, const vector<MapPoint*> &vpMapPoints, const float th, const bool bFarPoints, const float thFarPoints)
{
    ...
    for(size_t iMP=0; iMP<vpMapPoints.size(); iMP++) // Step 1 遍历有效的局部地图点
    {
        ...
        if(pMP->mbTrackInView)
        {
            ...
            if(bFactor) r*=th;// 如果需要扩大范围搜索，则乘以阈值th
            // Step 3 通过投影点以及搜索窗口和预测的尺度进行搜索, 找出搜索半径内的候选匹配点索引
            const vector<size_t> vIndices =
                    F.GetFeaturesInArea(pMP->mTrackProjX,pMP->mTrackProjY,r*F.mvScaleFactors[nPredictedLevel],nPredictedLevel-1,nPredictedLevel);   
            if(!vIndices.empty())// 没找到候选的,就放弃对当前点的匹配
            {
                const cv::Mat MPdescriptor = pMP->GetDescriptor();//当前地图点的描述子
                int bestDist=256;// 最优次优的描述子距离和index
                int bestLevel= -1;
                int bestDist2=256;
                int bestLevel2 = -1;
                int bestIdx =-1 ;
                ...
                for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++)
                {
                    ...
                    if(F.mvpMapPoints[idx]) if(F.mvpMapPoints[idx]->Observations()>0) continue;
                    if(F.Nleft == -1 && F.mvuRight[idx]>0)//如果是针孔相机模型的双目数据
                    {
                        const float er = fabs(pMP->mTrackProjXR-F.mvuRight[idx]);//计算在X轴上的投影误差
                        //超过阈值,说明这个点不行,丢掉.
                    ...
                    const int dist = DescriptorDistance(MPdescriptor,d);// 计算地图点和候选投影点的描述子距离
                    if(dist<bestDist) // 寻找描述子距离最小和次小的特征点和索引
                    {
                        bestDist2=bestDist;
                        bestDist=dist;
                ...
                // Step 5 筛选最佳匹配点最佳匹配距离还需要满足在设定阈值内
                if(bestDist<=TH_HIGH)
                {
...
int ORBmatcher::SearchByBoW(KeyFrame* pKF,Frame &F, vector<MapPoint*> &vpMapPointMatches)
{
    ...
    while(KFit != KFend && Fit != Fend)
    {
        ...
        if(KFit->first == Fit->first) // first 元素就是node id，遍历
        {
            ...
            for(size_t iKF=0; iKF<vIndicesKF.size(); iKF++)// Step 2：遍历KF中属于该node的特征点
            {
                const unsigned int realIdxKF = vIndicesKF[iKF];// 关键帧该节点中特征点的索引
                MapPoint* pMP = vpMapPointsKF[realIdxKF];// 取出KF中该特征对应的地图点
                if(!pMP || pMP->isBad()) continue;
                const cv::Mat &dKF= pKF->mDescriptors.row(realIdxKF);  // 取出关键帧KF中该特征对应的描述子
                int bestDist1=256; // 最好的距离（最小距离）
                int bestIdxF =-1 ;//最好距离的ID
                int bestDist2=256; // 次好距离（倒数第二小距离）
                int bestDist1R=256;// 双目中右图像最好的距离（最小距离）
                int bestIdxFR =-1 ;
                int bestDist2R=256;
                for(size_t iF=0; iF<vIndicesF.size(); iF++) // Step 3：遍历F中属于该node的特征点，寻找最佳匹配点
                {
                    if(F.Nleft == -1)//单目
                    {
                        const unsigned int realIdxF = vIndicesF[iF];// 这里的realIdxF是指普通帧该节点中特征点的索引
                        if(vpMapPointMatches[realIdxF]) continue;// 如果地图点存在，说明这个点已经被匹配过了，不再匹配，加快速度
                        const cv::Mat &dF = F.mDescriptors.row(realIdxF);// 取出普通帧F中该特征对应的描述子
                        const int dist =  DescriptorDistance(dKF,dF);// 计算描述子的距离
                        if(dist<bestDist1)//记录最佳距离、最佳距离对应的索引、次佳距离等，dist<bestDist1<bestDist2，更新bestDist1 bestDist2
                        {
                            bestDist2=bestDist1;
                            bestDist1=dist;
                            bestIdxF=realIdxF;
                        }
                        else if(dist<bestDist2) bestDist2=dist;// 如果bestDist1 < dist < bestDist2，更新bestDist2
                    }
                    else//双目
                    {
                ...
                if(bestDist1<=TH_LOW) // Step 4.1：第一关筛选：匹配距离必须小于设定阈值
                {
                    // Step 4.2：第二关筛选：最佳匹配比次佳匹配明显要好，那么最佳匹配才真正靠谱
                    if(static_cast<float>(bestDist1)<mfNNratio*static_cast<float>(bestDist2))
                    {
                        vpMapPointMatches[bestIdxF]=pMP;// Step 4.3：记录成功匹配特征点的对应的地图点(来自关键帧)
                        ...
                        if(mbCheckOrientation)// Step 4.4：计算匹配点旋转角度差所在的直方图
                        {
                            ...
                            float rot = kp.angle-Fkp.angle;// 所有的特征点的角度变化应该是一致的，通过直方图统计得到最准确的角度变化值
        ...
        else if(KFit->first < Fit->first) KFit = vFeatVecKF.lower_bound(Fit->first);// 对齐
        else Fit = F.mFeatVec.lower_bound(KFit->first);// 对齐
    }
    if(mbCheckOrientation)  // Step 5 根据方向剔除误匹配的点
    {
        int ind1=-1;
        int ind2=-1;
        int ind3=-1;
        ComputeThreeMaxima(rotHist,HISTO_LENGTH,ind1,ind2,ind3);  // 筛选出在旋转角度差落在在直方图区间内数量最多的前三个bin的索引
        for(int i=0; i<HISTO_LENGTH; i++)
        {
            if(i==ind1 || i==ind2 || i==ind3) continue;// 如果特征点的旋转角度变化量属于这三个组，则保留
 ...
 * @param F1 单目初始化第一帧
 * @param F2 单目初始化第二帧
 * @param vbPrevMatched 在初始化的过程中,保存参考帧（F1）中的特征点坐标
 * @param vnMatches12 保存参考帧F1中特征点是否匹配上，index保存是F1对应特征点索引，值保存的是匹配好的F2特征点索引
 * @param windowSize 匹配时的搜索半径大小
*/
int ORBmatcher::SearchForInitialization(Frame &F1, Frame &F2, vector<cv::Point2f> &vbPrevMatched, vector<int> &vnMatches12, int windowSize)
{
    int nmatches=0;
    vnMatches12 = vector<int>(F1.mvKeysUn.size(),-1);// F1中特征点和F2中匹配关系，注意是按照F1特征点数目分配空间
    vector<int> rotHist[HISTO_LENGTH]; // Step 1 构建旋转直方图，HISTO_LENGTH = 30
    for(int i=0;i<HISTO_LENGTH;i++) rotHist[i].reserve(500);// 每个bin里预分配500个，因为使用的是vector不够的话可以自动扩展容量
    //! 原作者代码是 const float factor = 1.0f/HISTO_LENGTH; 是错误的，更改为下面代码
    // const float factor = HISTO_LENGTH/360.0f;
    const float factor = 1.0f/HISTO_LENGTH;
    vector<int> vMatchedDistance(F2.mvKeysUn.size(),INT_MAX);// 匹配点对距离，注意是按照F2特征点数目分配空间
    vector<int> vnMatches21(F2.mvKeysUn.size(),-1);// 从帧2到帧1的反向匹配，注意是按照F2特征点数目分配空间
    for(size_t i1=0, iend1=F1.mvKeysUn.size(); i1<iend1; i1++)// 遍历帧1中的所有特征点
    {
        cv::KeyPoint kp1 = F1.mvKeysUn[i1];
        int level1 = kp1.octave;
        if(level1>0) continue;// 只使用原始图像上提取的特征点
        // Step 2 在半径窗口内搜索当前帧F2中所有的候选匹配特征点 
        vector<size_t> vIndices2 = F2.GetFeaturesInArea(vbPrevMatched[i1].x,vbPrevMatched[i1].y, windowSize,level1,level1);
        if(vIndices2.empty()) continue; // 没有候选特征点，跳过
        ...
        if(bestDist<=TH_LOW)// 即使算出了最佳描述子匹配距离，也不一定保证配对成功。要小于设定阈值50
        {
            if(bestDist<(float)bestDist2*mfNNratio)// 最佳距离比次佳距离要小于设定的比例，这样特征点辨识度更高
            {
                if(vnMatches21[bestIdx2]>=0)// 如果找到的候选特征点对应F1中特征点已经匹配过了，说明发生了重复匹配，将原来的匹配也删掉
                {
                ...
                nmatches++;
                if(mbCheckOrientation) // Step 5 计算匹配点旋转角度差所在的直方图
                {
                    float rot = F1.mvKeysUn[i1].angle-F2.mvKeysUn[bestIdx2].angle;// 计算匹配特征点的角度差，这里单位是角度°，不是弧度
                    if(rot<0.0) rot+=360.0f;
                    // 前面factor = HISTO_LENGTH/360.0f 
                    // bin = rot / 360.of * HISTO_LENGTH 表示当前rot被分配在第几个直方图bin  
                    int bin = round(rot*factor);
                    if(bin==HISTO_LENGTH) bin=0;// 如果bin 满了又是一个轮回
                    assert(bin>=0 && bin<HISTO_LENGTH);
    ...
    if(mbCheckOrientation)// Step 6 筛除旋转直方图中“非主流”部分
    {
        int ind1=-1;
        int ind2=-1;
        int ind3=-1;
...
{
    int nmatches = 0;
    vector<int> rotHist[HISTO_LENGTH];// Step 1 建立旋转直方图，用于检测旋转一致性
    for(int i=0;i<HISTO_LENGTH;i++) rotHist[i].reserve(500);
    //! 原作者代码是 const float factor = 1.0f/HISTO_LENGTH; 是错误的，更改为下面代码
    // const float factor = HISTO_LENGTH/360.0f;
    const float factor = 1.0f/HISTO_LENGTH;
    ...
    for(int i=0; i<LastFrame.N; i++)
    {
        ...
        if(pMP)
        {
            if(!LastFrame.mvbOutlier[i])// 对上一帧有效的MapPoints投影到当前帧坐标系
            {
                ...
                if(invzc<0) continue;
                Eigen::Vector2f uv = CurrentFrame.mpCamera->project(x3Dc);// 投影到当前帧中
                ...
                float radius = th*CurrentFrame.mvScaleFactors[nLastOctave]; // 尺度越大，搜索范围越大  // 单目：th = 7，双目：th = 15
                vector<size_t> vIndices2;// 记录候选匹配点的id
                // Step 4 根据相机的前后前进方向来判断搜索尺度范围。
                // 以下可以这么理解，例如一个有一定面积的圆点，在某个尺度n下它是一个特征点
                // 当相机前进时，圆点的面积增大，在某个尺度m下它是一个特征点，面积增大需要在更高的尺度下才能检测出来。上一帧兴趣点在所在的尺度nLastOctave<=nCurOctave
                // 当相机后退时，圆点的面积减小，在某个尺度m下它是一个特征点，面积减小需要在更低的尺度下才能检测出来。上一帧兴趣点在所在的尺度0<=nCurOctave<=nLastOctave
                if(bForward) vIndices2 = CurrentFrame.GetFeaturesInArea(uv(0),uv(1), radius, nLastOctave);
                else if(bBackward) vIndices2 = CurrentFrame.GetFeaturesInArea(uv(0),uv(1), radius, 0, nLastOctave);
                else vIndices2=CurrentFrame.GetFeaturesInArea(uv(0),uv(1),radius,nLastOctave-1,nLastOctave+1);//在[nLastOctave-1,nLastOctave+1]中搜索
                ...
                {
                    const size_t i2 = *vit;
                    if(CurrentFrame.mvpMapPoints[i2])// 如果该特征点已经有对应的MapPoint了,则退出该次循环
                        if(CurrentFrame.mvpMapPoints[i2]->Observations()>0) continue;
                    if(CurrentFrame.Nleft == -1 && CurrentFrame.mvuRight[i2]>0)
                    {
                        const float ur = uv(0) - CurrentFrame.mbf*invzc;
                        const float er = fabs(ur - CurrentFrame.mvuRight[i2]);
                        if(er>radius) continue;// 双目和rgbd的情况，需要保证右图的点也在搜索半径以内
                    }
                ...
                if(bestDist<=TH_HIGH)// 最佳匹配距离要小于设定阈值
                {
                    CurrentFrame.mvpMapPoints[bestIdx2]=pMP;
                    nmatches++;
                    if(mbCheckOrientation) // Step 6 计算匹配点旋转角度差所在的直方图
                    {
                ...
                if(CurrentFrame.Nleft != -1)//双目KannalaBrandt8模型
                {
                    ...
                    if(bForward) vIndices2 = CurrentFrame.GetFeaturesInArea(uv(0),uv(1), radius, nLastOctave, -1,true);
                    else if(bBackward) vIndices2 = CurrentFrame.GetFeaturesInArea(uv(0),uv(1), radius, 0, nLastOctave, true);
                    else vIndices2 = CurrentFrame.GetFeaturesInArea(uv(0),uv(1), radius, nLastOctave-1, nLastOctave+1, true);
                    const cv::Mat dMP = pMP->GetDescriptor();
                    int bestDist = 256;
                    int bestIdx2 = -1;
                    for(vector<size_t>::const_iterator vit=vIndices2.begin(), vend=vIndices2.end(); vit!=vend; vit++)
    ...
    if(mbCheckOrientation)//  Step 7 进行旋转一致检测，剔除不一致的匹配
    {
        int ind1=-1;
        int ind2=-1;
        int ind3=-1;
        ...
        for(int i=0; i<HISTO_LENGTH; i++)
        {
            if(i!=ind1 && i!=ind2 && i!=ind3) // 对于数量不是前3个的点对，剔除
            {
                for(size_t j=0, jend=rotHist[i].size(); j<jend; j++)
                {
