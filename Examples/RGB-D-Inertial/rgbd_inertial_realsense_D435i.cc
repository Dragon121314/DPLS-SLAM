...
{
    // Sensors usually have several options to control their properties，such as Exposure, Brightness etc.
...
int main(int argc, char **argv) 
{
    if (argc < 3 || argc > 4) 
    {
        cerr << endl << "Usage: ./mono_inertial_realsense_D435i path_to_vocabulary path_to_settings (trajectory_file_name)"  << endl;
        return 1;
    }
    string file_name;
    ...
    while (!SLAM.isShutDown())
    {
        ...
        {
            ORB_SLAM3::IMU::Point lastPoint(vAccel[i].x, vAccel[i].y, vAccel[i].z,vGyro[i].x, vGyro[i].y, vGyro[i].z,vGyro_times[i]);
...
#endif
        SLAM.TrackRGBD(im, depth, timestamp, vImuMeas);// Pass the image to the SLAM system
#ifdef REGISTER_TIMES
