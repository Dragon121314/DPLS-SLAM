...
int main(int argc, char **argv)
{
    ...
    cout << endl << "-----------------------------------" << endl;
    cout << "Start processing sequence , Images in the sequence: " << nImages << endl << endl;   
...
void LoadImages(const string &strPathToSequence, vector<string> &vstrImageLeft,vector<string> &vstrImageRight, vector<double> &vTimestamps)
{
    ...
    {
        stringstream ss;
        ss << setfill('0') << setw(6) << i;//设置填充字符为'0'。使用setw指定宽度，如果输出的字符串长度小于指定的宽度，setfill指定的字符将用于填充额外的空间。
