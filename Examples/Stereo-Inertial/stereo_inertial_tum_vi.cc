...
void LoadImagesTUMVI(const string &strPathLeft, const string &strPathRight, const string &strPathTimes,vector<string> &vstrImageLeft, vector<string> &vstrImageRight, vector<double> &vTimeStamps);
...
int main(int argc, char **argv)
{
    ...
    double t_track = 0.f;
    int proccIm = 0;
    for (seq = 0; seq<num_seq; seq++)// Main loop
    {
        cv::Mat imLeft, imRight;
        vector<ORB_SLAM3::IMU::Point> vImuMeas;
        proccIm = 0;
        //CLAHE 是一种图像对比度增强技术，3是对比度增强的强度，值越大，图像的对比度增强就越强，但可能导致更多的噪声。cv::Size(8, 8)指定了用于均衡化的区域大小
        ...
        for(int ni=0; ni<nImages[seq]; ni++, proccIm++)
        {
            ...
            // clahe
            clahe->apply(imLeft,imLeft);//调用 CLAHE 对象的 apply 方法，原始图像会被对比度增强后的图像替换。
            ...
            double T=0;
            if(ni<nImages[seq]-1) T = vTimestampsCam[seq][ni+1]-tframe;
            else if(ni>0) T = tframe-vTimestampsCam[seq][ni-1];
    ...
    float totaltime = 0;
    for(int ni=0; ni<nImages[0]; ni++) totaltime+=vTimesTrack[ni];
    cout << "------------------------------------------------" << endl << endl;
...
void LoadImagesTUMVI(const string &strPathLeft, const string &strPathRight, const string &strPathTimes,vector<string> &vstrImageLeft, vector<string> &vstrImageRight, vector<double> &vTimeStamps)
{
    ...
    while(!fTimes.eof())
    {
        ...
        {
            if (s[0] == '#') continue;
            int pos = s.find(' ');//在字符串s中查找第一个空格字符（' '）的位置，并将其存储在变量 pos 中。
            string item = s.substr(0, pos);//使用substr方法从字符串s中提取从开始到位置pos（不包括pos）的子字符串
            vstrImageLeft.push_back(strPathLeft + "/" + item + ".png");
            vstrImageRight.push_back(strPathRight + "/" + item + ".png");
            double t = stod(item);//使用stod函数（string-to-double）将item字符串转换为double类型的数值，这个数值代表纳秒级的时间戳。
...
void LoadIMU(const string &strImuPath, vector<double> &vTimeStamps, vector<cv::Point3f> &vAcc, vector<cv::Point3f> &vGyro)
{
    ...
    while(!fImu.eof())
    {
        ...
        if(!s.empty())
        {
            ...
            double data[7];
            int count = 0;
            while ((pos = s.find(',')) != string::npos) //直到find方法返回string::npos，即字符串中没有逗号可以找到
            {
                item = s.substr(0, pos);
