...
int main(int argc, char **argv)
{
    ...
    vector<int> nImages;
    vector<int> nImu;
    vector<int> first_imu(num_seq,0);//imu小于图像帧的时间戳索引记录
    ...
    for (seq = 0; seq<num_seq; seq++)
    {
        ...
        // for(int ni=0; ni<nImages[seq]; ni++, proccIm++)//保存回环图像
        // {
        //     double tframe = vTimestampsCam[seq][ni];
        //     vector<double> cur=SLAM.getcurrenttime();
        //     vector<double> math=SLAM.getmatchedtime();

        //     bool curt=find(cur.begin(),cur.end(),tframe)!=cur.end();
        //     bool mat=find(math.begin(),math.end(),tframe)!=math.end();
        //     if(curt || mat)
        //     {
        //         bool isaved=cv::imwrite("/home/robot/Temperoy/img/"+to_string(tframe)+".png",imLeft);
        //         if(!isaved) {std::cerr << "Failed to save the image." << std::endl;return 1;}
        //     }
        // }
